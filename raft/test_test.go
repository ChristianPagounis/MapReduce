...
func TestReElection2A(t *testing.T) {
	...
	fmt.Println("FROM TEST: Disconnecting1", leader1)
	cfg.checkOneLeader()

	// if the old leader rejoins, that shouldn't
	// disturb the new leader.
	cfg.connect(leader1)
	fmt.Println("FROM TEST: Connect1-1", leader1)
	leader2 := cfg.checkOneLeader()

	fmt.Println("FROM TEST: leader selected", leader1)
	// if there's no quorum, no leader should
	// be elected.
	cfg.disconnect(leader2)
	fmt.Println("FROM TEST: Disconnecting2", leader2)
	cfg.disconnect((leader2 + 1) % servers)
	fmt.Println("FROM TEST: Disconnecting3", (leader2+1)%servers)
	time.Sleep(2 * RaftElectionTimeout)
	cfg.checkNoLeader()

	// if a quorum arises, it should elect a leader.
	cfg.connect((leader2 + 1) % servers)
	fmt.Println("FROM TEST: Connect3-1", (leader2+1)%servers)
	cfg.checkOneLeader()

	// re-join of last node shouldn't prevent leader from existing.
	cfg.connect(leader2)
	fmt.Println("FROM TEST: Connecting3-2", leader2)
	cfg.checkOneLeader()
